...
export async function GET(request: Request) {
  try {
    ...
    // Modified query to find programs where rucEntidad array contains the requested rucEntidad
    const programas = await db
      .collection("programas")
      .find({ rucEntidad: { $in: [rucEntidad] } })
      .sort({ numero: 1 })
      .toArray()
    ...
    return NextResponse.json({ error: "Error al obtener los programas" }, { status: 500 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    // Ensure rucEntidad is always an array
    const rucEntidad = Array.isArray(json.rucEntidad) ? json.rucEntidad : [json.rucEntidad]
    ...
    const result = await db.collection("programas").insertOne(programa)
